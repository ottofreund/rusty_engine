...
impl Board {
    ...
    pub fn get_piece_type_at(&self, sqr: u32, owner: u32) -> u32 {
        return self
            .lift_piece_type_at(sqr, owner)
            .expect("sqr wasn't occupied although expected");
    }
    ...
    pub fn update_castling_rights_make(
        &mut self,
        from: u32,
        to: u32,
        is_white_turn: bool,
        ...
        if is_white_turn {
            short_dist = &mut self.ws;
            long_dist = &mut self.wl;
            short_corner_idx = 7;
            long_corner_idx = 0;
            king_piece_idx = W_KING;
            non_mover_s_dist = &mut self.bs;
            non_mover_l_dist = &mut self.bl;
            non_mover_s_corner_idx = 63;
            non_mover_l_corner_idx = 56;
        } else {
            short_dist = &mut self.bs;
            long_dist = &mut self.bl;
            short_corner_idx = 63;
            long_corner_idx = 56;
            king_piece_idx = B_KING;
            non_mover_s_dist = &mut self.ws;
            non_mover_l_dist = &mut self.wl;
            non_mover_s_corner_idx = 7;
            non_mover_l_corner_idx = 0;
        }
        ...
        if moved_piece == king_piece_idx {
            ...
            *long_dist += 1;
            ...
            if !short_right || from == short_corner_idx || to == short_corner_idx {
                //check if from or to corresponding corner or already lost short right
                *short_dist += 1;
            }
    ...
    pub fn board_with(
        pieces: [u64; 12],
        white_occupation: u64,
        black_occupation: u64,
        turn: u32,
        ws: u32,
        wl: u32,
        bs: u32,
        ...
        let mut res: Board = Self {
            ...
            white_pinned: 0,
            black_pinned: 0,
            white_pinned_restrictions: [0u64; 64],
            black_pinned_restrictions: [0u64; 64],
            nof_checkers: 0,
            check_block_sqrs: 0,
            meta_attacks: 0,
            major_minor_count,
        ...
        if turn == WHITE {
            res.black_attacks = non_mover_attacks
        } else {
