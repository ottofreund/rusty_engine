...
impl MagicBitboard {
    ...
    pub fn init_magic(
        empty_board_attack_bbs: &[[u64; 64]; 12],
        rook_empty_board_attack_bbs_no_edges: &[u64; 64],
        bishop_empty_board_attack_bbs_no_edges: &[u64; 64],
        new_magics: bool,
    ) -> Self {
        ...
        for sqr in 0..64 {
            ...
            let all_rook_block_masks: Vec<u64> = move_gen::generate_all_blocker_masks(
                rook_empty_board_attack_bb,
                Some(rook_empty_board_attack_bb_no_edges),
            );
            let all_bishop_block_masks: Vec<u64> = move_gen::generate_all_blocker_masks(
                bishop_empty_board_attack_bb,
                Some(bishop_empty_board_attack_bb_no_edges),
            );
            ...
            if new_magics {
                ...
                find_working(
                    ...
                    false,
                    &mut bishop_magics,
                    &all_bishop_block_masks,
                    &mut rng,
                );
            }
            //form the magic indexed vector(s) for this sqr with legal slides
            let mut rook_lookup_vec: Vec<u64> = vec![0u64; 1 << ROOK_BITS[sqr]];
            for block_mask in all_rook_block_masks {
                //for rook
                let magic_idx: usize =
                    ((block_mask.wrapping_mul(rook_magics[sqr])) >> ROOK_SHIFTS[sqr]) as usize;
            ...
            for block_mask in all_bishop_block_masks {
                //for bishop
                let magic_idx: usize =
                    ((block_mask.wrapping_mul(bishop_magics[sqr])) >> BISHOP_SHIFTS[sqr]) as usize;
                bishop_lookup_vec[magic_idx] = naive_bishop_sliding(sqr as u32, block_mask, true);
            }
            cur_rook_offset += rook_lookup_vec.len();
            cur_bishop_offset += bishop_lookup_vec.len();
    ...
    pub fn get_magic_idx(&self, sqr: usize, rel_blockers: u64, rook: bool) -> usize {
        ...
        if rook {
            magic =
                ((rel_blockers.wrapping_mul(self.rook_magics[sqr])) >> ROOK_SHIFTS[sqr]) as usize;
            sqr_offset = self.rook_sqr_offset[sqr];
        } else {
            magic = ((rel_blockers.wrapping_mul(self.bishop_magics[sqr])) >> BISHOP_SHIFTS[sqr])
                as usize;
...
fn find_working(
    sqr: u32,
    rook: bool,
    magic_arr: &mut [u64; 64],
    all_block_masks: &Vec<u64>,
    rng: &mut ThreadRng,
) {
    let bits_needed: u32;
    if rook {
        bits_needed = ROOK_BITS[sqr as usize];
    } else {
        bits_needed = BISHOP_BITS[sqr as usize];
    }
    ...
    loop {
        ...
        for block_mask in all_block_masks {
            ...
            if used[magic_idx] {
                //collision, this magic doesn't work
                collided = true;
                break;
            }
...
const ROOK_MAGICS_MEMOIZED: [u64; 64] = [
    36028866812190857,
    918751921001480195,
    180152849915977728,
    36033745089742848,
    ...
    72061994240313344,
    1173750867632328724,
    13835111123898598913,
    11529637269273067776,
    9372167021304744576,
    288799374526391557,
    3603477849257808192,
    18156544822611968,
...
const BISHOP_MAGICS_MEMOIZED: [u64; 64] = [
    4543491443261712,
    1172062919782072705,
    24806807717281921,
    1180093748382557265,
